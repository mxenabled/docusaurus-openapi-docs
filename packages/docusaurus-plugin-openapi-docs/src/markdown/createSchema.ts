...
function createAdditionalProperties(schema: SchemaObject) {
  ...
  if (String(additionalProperties) === "true" && schema.type === "object") {
    // MX changes: remove creation of "SchemaItem" node, just return what would've been its props
    return {
      ...sharedConfig,
      schemaName: "any",
 ...
 * For handling items.
 */
// MX changes: remove opening and closing array brackets from return arrays
function createItems(schema: SchemaObject) {
  ...
  if (schema.items?.allOf !== undefined) {
    ...
    if (
      ...
      return [createAnyOneOf(mergedSchemas)].flat();
    }

    // Handles properties
    if (mergedSchemas.properties !== undefined) {
      return [createProperties(mergedSchemas)].flat();
    }
  }
  ...
  if (
    ...
    return [createNodes(schema.items)].flat();
  }
 ...
 */
// MX changes: remove creation of "SchemaItem" node, just return data that can be used in a single component
// This makes it easier to customize the component via swizzling
// because the layout is separated from the data
...
function createAnyOneOfProperty(
  ...
  const qualifierMessage = getQualifierMessage(schema);
  const type = schema.oneOf ? "oneOf" : "anyOf";
  const children = schema[type] || [];
  const possibleValues = children.map((property, i) => {
    const label = property.title ?? `MOD${i + 1}`;
...
function createPropertyDiscriminator(
  ...
  // MX changes: remove creation of "SchemaItem" node, just return the data
  return {
    field: name,
    type: schemaName,
...
function createEdges({
  ...
  if (schema.oneOf !== undefined || schema.anyOf !== undefined) {
    return createAnyOneOfProperty(name, schemaName, schema, required);
  }
  ...
  if (schema.type !== undefined) {
    if (schema.allOf) {
      ...
      if (schema.allOf.length && typeof schema.allOf[0] === "string") {
        // MX TODO: don't return a component here, just return the data
        return create("div", {
          style: {
            marginTop: ".5rem",
            marginBottom: ".5rem",
